 ...
 * @returns
 */
let webview = (id) => { return document.getElementById('page' + id) };

/**
 * @param {Number} id 
 * @returns
 */
let tab = (id) => { return document.getElementById('tab' + id) }
...
function addEvents(id) {
    ...
    wv.addEventListener("did-start-loading", e => {
        ...
        if (e.url != undefined) url.textContent = e.url == 'New tab' ? e.url : urlParser(e.url).hostname;
