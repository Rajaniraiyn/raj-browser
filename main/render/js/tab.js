...
function newTab(url, favicon, title, hostname) {
  ...
  if (url == undefined) {
    tabC.insertAdjacentHTML(
      "beforeend",
      tabT(id, favicon, "New tab", "", "New tab")
    );
    ...
    tabC.insertAdjacentHTML(
      "beforeend",
      tabT(id, favicon, title, url, hostname)
    );
...
function closeTab(id) {
  ...
  setTimeout((_) => {
    ...
    if (tabs.length == 1) {
      return document
        .querySelector("#title-bar-buttons > img:nth-child(3)")
        .click();
    }
    ...
    if (tbIndex == 0) {
      ...
      document
        .getElementById(nextTab.id.replace("tab", "page"))
